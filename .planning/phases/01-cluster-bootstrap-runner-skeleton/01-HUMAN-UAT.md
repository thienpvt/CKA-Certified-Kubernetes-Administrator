...
latest: "After `ssh-keyscan -H worker-1 worker-2`, `ssh-copy-id -i ~/.ssh/cka_sim_ed25519.pub root@10.140.0.13` and `.14` both failed with `Permission denied (publickey)`. Bootstrap still stops at worker-1 pubkey distribution."
...
latest: "Host keys were added for worker-1 and worker-2. Remaining blocker is no accepted login path to append the pubkey on workers."
