...
# SSH preflight only. The question is about worker-1 static pods, so setup does
...
if ! ssh -o BatchMode=yes -o ConnectTimeout=5 worker-1 true 2>/dev/null; then
  echo "error: passwordless SSH to worker-1 failed -- run 'cka-sim doctor' and re-run 'cka-sim bootstrap' if needed" >&2
  exit 1
fi
...
# No in-cluster pre-seed; candidate drops the manifest into /etc/kubernetes/manifests on worker-1.
