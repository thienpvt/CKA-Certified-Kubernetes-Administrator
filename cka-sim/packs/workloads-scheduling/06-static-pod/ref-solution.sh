...
# Drops a static-pod manifest into /etc/kubernetes/manifests on worker-1 via SSH
# and waits for the kubelet to mirror it into the default namespace.
set -euo pipefail
: "${CKA_SIM_LAB_NS:?CKA_SIM_LAB_NS must be set}"

ssh -o BatchMode=yes worker-1 'sudo tee /etc/kubernetes/manifests/q06-static-nginx.yaml >/dev/null' <<'EOF'
apiVersion: v1
kind: Pod
metadata:
...
for _ in $(seq 1 12); do
  if kubectl get pod q06-static-nginx-worker-1 -n default -o name >/dev/null 2>&1; then
    break
  fi
  sleep 5
done

kubectl wait --for=condition=Ready pod/q06-static-nginx-worker-1 -n default --timeout=120s
