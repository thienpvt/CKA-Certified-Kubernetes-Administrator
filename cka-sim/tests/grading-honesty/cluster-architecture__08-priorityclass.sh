#!/bin/bash
# Phase 07.1 grading-honesty regression: cluster-architecture/08-priorityclass
# Asserts:
#   - empty submission (post-setup state) scores 0/2 (no candidate modification).
#   - ref-solution (post-ref-solution state) scores 2/2.
# Setup creates both q08-critical and q08-batch with globalDefault=false.
# Candidate patches q08-critical to globalDefault=true.
# Uses assert_changed_since_setup priorityclass q08-critical + globalDefault count.
...
export CKA_SIM_LAB_NS="cka-sim-cluster-architecture-08"
