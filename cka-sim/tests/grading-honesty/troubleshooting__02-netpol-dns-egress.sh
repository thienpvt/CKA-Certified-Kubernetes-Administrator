#!/bin/bash
# Phase 07.1 grading-honesty regression: troubleshooting/02-netpol-dns-egress
# Asserts empty submission (post-setup state) scores 0/3 (NP unchanged + DNS+reach blocked by default-deny)
# AND ref-solution (post-ref-solution state) scores 3/3 (NP edited, DNS+reach allowed).
...
export CKA_SIM_LAB_NS="cka-sim-troubleshooting-02"
