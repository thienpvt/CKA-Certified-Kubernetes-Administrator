#!/bin/bash
# Phase 7 UAT — run all manual test cases on the live cluster.
...
#
# This script runs a quick 3-question exam using the real blueprint-alpha,
# exercises signals, resume, score, and list history — then reports results.
...
# ─────────────────────────────────────────────────────────────────────
# Test 4: Score report generation (uses mock exam run)
# ─────────────────────────────────────────────────────────────────────
info "Test 4: Score report (end-to-end with real graders)"
info "  Running a quick exam — auto-submitting all 17 questions..."

# Feed 17 Enter keystrokes (advance through all questions) + "y" to submit
# Use a short timeout override so the exam doesn't wait 2 hours
export CKA_SIM_NOW_OVERRIDE=""  # use real time

# Create input: 17 enters + y for submit
input=""
...
if [[ -n "$session_ts" ]]; then
  ...
  fail "Test 4a: exam session" "no session timestamp found in output"
...
if echo "$list_out" | grep -qE "(blueprint-alpha|mock-alpha|Started|No exam history)"; then
...
# ─────────────────────────────────────────────────────────────────────
# Test 1 & 2: Timer + Signals (informational — requires interactive terminal)
# ─────────────────────────────────────────────────────────────────────
info "Test 1 & 2: Timer + Signals"
info "  These tests require an interactive terminal session."
info "  To verify manually:"
info "    1. Run: bash $CKA_SIM exam blueprint-alpha"
info "    2. Observe timer at bottom of screen (updates every second)"
info "    3. Press Ctrl-C — should flag question, NOT kill exam"
info "    4. Press Ctrl-Z — should pause; type 'fg' to resume"
info "    5. Verify timer resumes with correct remaining time"
info ""
info "  Skipping automated check (interactive signals can't be tested in a script)."
total=$((total+2))  # count as skipped, not failed
...
if (( failed == 0 )); then
  ok "All automated UAT tests passed!"
  info "Manual tests (timer + signals) require interactive verification."
  exit 0
else
